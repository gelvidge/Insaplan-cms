CREATE TYPE "public"."enum_users_role" AS ENUM('admin', 'editor', 'viewer');
  ...
  -- Optional grants for the app DB role used by this repo's `DATABASE_URL`.
  -- Some Supabase pooler usernames are not real Postgres roles, so attempting to GRANT can fail.
  -- If you need grants, set `app_role` below to a role that exists in your DB (check `pg_roles`).
  DO $$
  DECLARE
    app_role text := 'postgres.ytbtbnfxxdjrblyffbph';
  BEGIN
    ...
    ELSE
      RAISE NOTICE 'Role "%" does not exist; skipping GRANT statements', app_role;
    END IF;
  END $$;
